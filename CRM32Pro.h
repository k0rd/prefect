...
#define RecoverSurf         // By default, auto recover surfaces support is enabled.
    ...
    #define _LINUX                               // Is a linux platform?
    #define DllHandle   
    #define DllExport  
    #undef RecoverSurf                           // In Linux, auto recover support is not needed.
#elif defined(__MACOSX__)
    #define _MACOSX                              // Is a MacOSX platform?
    #define DllHandle   
    #define DllExport  
    #undef RecoverSurf                           // In MacOSX, auto recover support is not needed.
...
// ---------------------------------------------------------------------------
/** @defgroup MODULE1 MODULE 1: CRM32Pro main and generic functions (v2.5). 
...
#define RENDER_DIRECTX  4 //!< Used with Config.VideoRenderer to use DirectX backend. Only for Win32. Deprecated.
#define RENDER_X11      8 //!< Used with Config.VideoRenderer to use X11 backend. Only for Linux/MacOSX.
#define RENDER_FBCON   16 //!< Used with Config.VideoRenderer to use Frame Buffer Console backend. Only for Linux. Deprecated
#define RENDER_OPENGL  64 //!< Used with Config.VideoRenderer to use glSDL backend(OpenGL on 2D mode). For Win32/Win64, Linux and MacOSX.
#define RENDER_QUARTZ 128 //!< Used with Config.VideoRenderer to use Quartz backend. Only for MacOSX.
...
#define EVENT_CODE_RENDER   1  //!< When the event returned is EVENT_LOGICWAIT, if the event.user.code is equal to EVENT_CODE_RENDER sets that a Render frame occurred.
...
// -Flags for RenderNeeded() method-
#define UPDATEFRAME_ALWAYS 2 //!< Used with RenderNeeded() method. Set the render engine to full redraw on each frame
#define UPDATEFRAME_NEXT   1 //!< Used with RenderNeeded() method. Set the render engine to draw on next frame update
#define UPDATEFRAME_NONE   0 //!< Used with RenderNeeded() method. Set the render engine to avoid to draw on next frame update

// -Flags for ConFeatures() method-
#define CONFEATURE_DISABLE_KEY   1 //!< Used with ConFeatures() method. Disables the ~ key for displaying/hiding the console
#define CONFEATURE_OUTPUT_ONLY   2 //!< Used with ConFeatures() method. Disables the console input so it will just display output messages
#define CONFEATURE_DISABLE_FADE  4 //!< Used with ConFeatures() method. Disables the fading effect when opening or closing the console
...
class DllExport CRM32Pro_Main 
{
    public:
		...
		// -Check if glSDL is enabled-
		int IsGL();
		// -Convert surface into a texture (glSDL)-
		int Surface2Texture(SDL_Surface *surf);
		// -Is a full render needed?-
		int IsRenderNeeded();
		// -Enable a full render-
		...
		// -Integrated Debug Window system-
		void DwTitle(char *);
		void DwEnable();
		void DwDisable();
		Uint8 DwisEnabled();
		...
		struct sConfig
		{
			...
			Uint8 VideoWindow;   //!< 0 for fullscreen. 1 for window mode. 2 for window mode without frame. By default is 1.
		...
		Uint8 mouse_buttons;     /**< @brief Current state of mouse's buttons. Updated automatically in CRM32Pro.Update().
        ...
        Uint8 mouse_prevbuttons; //!< Previous state of mouse's buttons. Updated automatically in CRM32Pro.Update().
...
// ---------------------------------------------------------------------------
/** @defgroup MODULE2 MODULE 2: ILogSystem v2.7, log system interface.
...
//! @{
   ...
   #define LOG_DRTS              16 //!< Use with Init() in 'mode' parameter: output to DRTS subsystem (integrated console)
...
// ---------------------------------------------------------------------------
/** @defgroup MODULE3 MODULE 3: IStuffDPF v4.31, Data Package Format interface.
...
// ---------------------------------------------------------------------------
/** @defgroup MODULE4 MODULE 4: IImage v3.13, Image Package Format interface.
...
// ---------------------------------------------------------------------------
/** @defgroup MODULE6 MODULE 6: ICursor v2.82, manage cursor actions.
...
// ---------------------------------------------------------------------------
/** @defgroup MODULE7 MODULE 7: CTile v2.12, tile engine class.
 ...
 - Last modified on 1 November 2009*/
// ---------------------------------------------------------------------------
//! @{
...
// ---------------------------------------------------------------------------
/** @defgroup MODULE8 MODULE 8: CFont v2.73, font engine class.
...
class DllExport CRM32Pro_CFont
{
    public:
		...
		// -Get the size(width) of the given string (in pixels)-
...
// ---------------------------------------------------------------------------
/** @defgroup MODULE9 MODULE 9: IScreenFX v4.70, screen FX interface.
 ...
 CRM32Pro.Init() builds this interface and CRM32Pro.Quit() removes it. <br>
 - Created on 9 July 2001
 - Last modified on 14 June 2011*/
// ---------------------------------------------------------------------------
//! @{
   #define FXSCREEN_NULL       0 //!< Switch off any kind of effect
   #define FXSCREEN_COLOR      1 //!< Color threshold effect Color threshold effect. Not supported in OpenGL modes.
   #define FXSCREEN_WIGGLE     2 //!< Wiggle effect
   #define FXSCREEN_ZOOM       3 //!< Progressive zoom effect. Not supported in OpenGL modes.
   #define FXSCREEN_VIDEO      4 //!< Video effect. Not supported in OpenGL modes.
   #define FXSCREEN_WIND       5 //!< Wind effect. Not supported in OpenGL modes.
   #define FXSCREEN_NEGATIVE   6 //!< Negative color effect. Not supported in OpenGL modes.
   #define FXSCREEN_GREY       7 //!< Grey effect. Not supported in OpenGL modes.
   #define FXSCREEN_GREYREAL   8 //!< Accurate grey effect. Not supported in OpenGL modes.
   #define FXSCREEN_BLACKWHITE 9 //!< Black/White effect. Not supported in OpenGL modes.
...
// ---------------------------------------------------------------------------
/** @defgroup MODULE10 MODULE 10: IButton v4.40, GUI interface.
 ...
 Currently it supports up to 8(MAX_LAYERBUTTON) layers with 32(MAX_BUTTON) maximum buttons per layer.<br>
...
// ---------------------------------------------------------------------------
   ...
   #define B_OFF 16    //!< Button state: disabled
   ...
   typedef struct IButtonNew                                       
	{                                                                  
	 ...
	 short int range;                  //!< Range. With Input buttons it stores the Y offset
	 char bVisible;                    //!< 0=invisible and 1=visible
	 char bClon;                       //!< Am I a clone? 0=no 1=yes
	 int cvalue;                       //!< Current value. With Input buttons it stores the X offset
   } IButtonNew;    
   ...
   {    
    ...
    public:
		...
		// -Draw buttons of current layer returning the ID of clicked button-
		int Draw(void);      
		// -Return the ID of clicked button-
		int Check(void);
...
// ---------------------------------------------------------------------------
/** @defgroup MODULE11 MODULE 11: ITimeSystem v1.80, timer system interface
 ...
 * - Last modified on 11 June 2011*/
// ---------------------------------------------------------------------------
//! @{
#define TIMER_INIT   0  //!< Used with Init(). It initializes the timer system. 
...
class DllExport CRM32Pro_ITimeSystem
   {
	   friend class CRM32Pro_Main;
	   friend class CRM32Pro_IVideo;
	   friend class CRM32Pro_IScreenFX;
    public:
		// -Constructor-
   ...
   private:
	   ...
	   void Update(); // Called from IScreenFX and IVideo too
	   void IncRF();
	   void IncLF();
...
// ---------------------------------------------------------------------------
/** @defgroup MODULE12 MODULE 12: CSprite v4.21, sprite engine class.
...
// ---------------------------------------------------------------------------
/** @defgroup MODULE13 MODULE 13: INetwork v4.21, network interface
...
// ---------------------------------------------------------------------------
/** @defgroup MODULE14 MODULE 14: IVideo v2.40, player video interface
 ...
 CRM32Pro.Init() builds this interface and CRM32Pro.Quit() removes it.  <br> <br>
 MPEG playback with OpenGL video modes will put a lot of stress on the full system (CPU,bus and video card) so it is not recommended.
 In case you use FX effects, it requires even a more powerful system to work fine (>1.2Ghz CPU).
 However, it will work but requires a special workaround, check \ref Example05_VideoPlayer.cpp "Example05_VideoPlayer" for further information. 
...
// ---------------------------------------------------------------------------
/** @defgroup MODULE15 MODULE 15: ISoundFX v4.20, sound,music and FX effects interface
